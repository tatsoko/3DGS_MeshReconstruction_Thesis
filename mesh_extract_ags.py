...
def extract_mesh(dataset, pipe, checkpoint_iterations=None):
    ...
    point_cloud_path = os.path.join(output_path,"iteration_" + str(iteration))
    
    gaussians.load_ply(os.path.join(point_cloud_path, "point_cloud.ply"), os.path.join(point_cloud_path, "params.npy"))
    if gaussians.brdf:
                fn = os.path.join(dataset.model_path,
                                "brdf_mlp",
                                "iteration_" + str(iteration),
                                "brdf_mlp.hdr")
                gaussians.brdf_mlp = load_env(fn, scale=1.0)
    ...
    o3d.io.write_triangle_mesh(os.path.join(dataset.model_path,"recon_tsdf_radegs.ply"),mesh.to_legacy())
    print("done!")
