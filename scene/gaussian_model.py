...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.brdf_envmap_res = 512  #move to params
    ...
    #TODO I think currently create from pvd is not used
    ...
    def construct_list_of_attributes_reformatted(self, viewer_fmt=False):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
        # All channels except the 3 DC
        for i in range(self._features_dc.shape[1]):
            l.append('f_dc_{}'.format(i))
    ...
    def save_ply_reformatted(self, path):
        xyz = self._xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz) if not self.brdf else self._normal.detach().cpu().numpy()
        ...
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes_reformatted()]
    ...
    # def save_additional_params_to_npy(self, path):
    #     mkdir_p(os.path.dirname(path))
    #     # Extract parameters
    #     normals = self._normal.detach().cpu().numpy()       # (nx, ny, nz)
    #     normals2 = self._normal2.detach().cpu().numpy()     # (nx2, ny2, nz2)
    #     specular = self._specular.detach().cpu().numpy()    # Specular values
    #     roughness = self._roughness.detach().cpu().numpy()  # Roughness values
    ...
    #     }
    #     # Save to a .npy file
    #     save_path = path if path.endswith('.npy') else f"{path}.npy"
    #     np.save(save_path, data)
    # def load_additional_params_from_npy(self, path):
    #     load_path = path if path.endswith('.npy') else f"{path}.npy" 
    #     # Load the data dictionary
    #     data = np.load(load_path, allow_pickle=True).item()
    #     # Convert data back to nn.Parameters with requires_grad=True
    #     self._normal = nn.Parameter(torch.tensor(data["normals"], dtype=torch.float, device="cuda").requires_grad_(True))
    #     self._normal2 = nn.Parameter(torch.tensor(data["normals2"], dtype=torch.float, device="cuda").requires_grad_(True))
    #     self._specular = nn.Parameter(torch.tensor(data["specular"], dtype=torch.float, device="cuda").requires_grad_(True))
    #     self._roughness = nn.Parameter(torch.tensor(data["roughness"], dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    # def save_ply(self, path, path_additional):
    #     mkdir_p(os.path.dirname(path))
    ...
    #     el = PlyElement.describe(elements, 'vertex')
    #     PlyData([el]).write(path)
    #     self.save_additional_params_to_npy(path_additional)
    ...
    # def load_ply(self, path, path_additional):
    #     plydata = PlyData.read(path)
    ...
    #     self.load_additional_params_from_npy(path_additional)
    #     self.active_sh_degree = self.max_sh_degree
